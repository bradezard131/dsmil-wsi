...
def get_bag_feats(
    csv_file_df: pd.Series, dataset: str, num_classes: int, shuffle_bag: bool = True
):
    if dataset == "TCGA-lung-default":
        feats_csv_path = (
            "datasets/tcga-dataset/tcga_lung_data_feats/"
            + str(csv_file_df.iloc[0].split("/")[1])
            + ".csv"
        )
    else:
        feats_csv_path = str(csv_file_df.iloc[0])
    df = pd.read_csv(feats_csv_path)
    if shuffle_bag:
        feats = cast(pd.DataFrame, shuffle(df).reset_index(drop=True))
    else:
    ...
    return label, feats_arr, feats_csv_path


def generate_pt_files(df: pd.DataFrame, dataset: str, num_classes: int):
    ...
    for i in tqdm(range(len(df))):
        label, feats, feats_csv_path = get_bag_feats(df.iloc[i], dataset, num_classes)
...
def train(
    ...
    for i, item in enumerate(dirs):
        ...
        bag_label = torch.as_tensor(
            stacked_data[0, feats_size:], device="cuda:0"
        ).unsqueeze(0)
        bag_feats = torch.as_tensor(stacked_data[:, :feats_size], device="cuda:0")
        bag_feats = dropout_patches(bag_feats, 1 - dropout_patch)
...
def test(
    ...
    with torch.no_grad():
        for i, item in enumerate(test_df):
            ...
            test_labels_l.extend([bag_label.squeeze().cpu().numpy().astype(int)])
            if average:
                test_predictions_l.extend(
                    [
                        (torch.sigmoid(max_prediction) + torch.sigmoid(bag_prediction))
                        .squeeze()
                        .cpu()
                        .numpy()
                    ]
    ...
    test_predictions = np.array(test_predictions_l)
    auc_value, _, thresholds_optimal = multi_label_roc(
        test_labels, test_predictions, num_classes
    )
    if thresholds:
...
def multi_label_roc(labels, predictions, num_classes):
    ...
    for c in range(0, num_classes):
        ...
        _fpr_optimal, _tpr_optimal, threshold_optimal = optimal_thresh(
            fpr, tpr, threshold
        )
        try:
...
def print_epoch_info(
    dataset, num_epochs, epoch, train_loss_bag, test_loss_bag, avg_score, aucs
):
    if dataset.startswith("TCGA-lung"):
        print(
        ...
        print(
            ...
            % (epoch, num_epochs, train_loss_bag, test_loss_bag, avg_score)
...
def apply_sparse_init(m: nn.Module) -> None:
...
def main(
    ...
    dataset: str = "TCGA-lung-default",
    split: float = 0.2,
    model: str = "dsmil",
    dropout_patch: float = 0,
    dropout_node: float = 0,
    non_linearity: float = 1,
    average: bool = False,
    eval_scheme: str = "5-fold-cv",
) -> None:
    ...
    if dataset == "TCGA-lung-default":
        ...
        bags_csv = os.path.join("datasets", dataset, dataset + ".csv")

    generate_pt_files(pd.read_csv(bags_csv), dataset, num_classes)
    ...
    if eval_scheme == "5-fold-cv":
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(bags_path)):
            ...
            milnet, criterion, optimizer, scheduler = get_fresh_model()
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                if current_score > fold_best_score:
                    ...
                    best_auc = aucs
                    save_model(
                        fold, run, save_path, milnet, thresholds_optimal, dataset
                    )
                if counter > stop_epochs:
                    break
    ...
    elif eval_scheme == "5-time-train+valid+test":
        bags_path = glob.glob("temp_train/*.pt")
        ...
        for iteration in range(5):
            ...
            train_end = int(total_samples * (1 - split - 0.1))
    ...
    if eval_scheme == "5-fold-cv-standalone-test":
        bags_path = glob.glob("temp_train/*.pt")
        bags_path = shuffle(bags_path)
        reserved_testing_bags = bags_path[: int(split * len(bags_path))]
        bags_path = bags_path[int(split * len(bags_path)) :]
        kf = KFold(n_splits=5, shuffle=True, random_state=42)
        fold_results = []
        fold_models = []
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(bags_path)):
            print(f"Starting CV fold {fold}.")
            milnet, criterion, optimizer, scheduler = init_model(
                feats_size=feats_size,
                num_classes=num_classes,
                dropout_node=dropout_node,
                non_linearity=non_linearity,
        ...
        if num_classes > 1:
            ...
            balanced_accuracy = balanced_accuracy_score(
                test_labels, combined_predictions
            )
